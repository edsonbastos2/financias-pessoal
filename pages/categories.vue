      ...
      <table class="mt-4 min-w-full divide-y divide-gray-200 shadow">
        ...
        <tr v-for="category in categories" :key="category.id" class="bg-white">
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
            <div v-if="category.is_update" class="w-72">
...
export default {
  ...
  async asyncData({store}){
    return {
      categories: await store.dispatch('categories/getCategories').then(resp => resp.map(obj => ({...obj, is_update:false})))
    }
  },
  ...
  methods: {
    ...
    updateCategory(category){
      ...
      this.$store.dispatch('categories/updateCategory', {id: category.id, data}).then(() => category.is_update = false)
    },
    addCategory(){
      if(!this.name) return
      ...
      this.$store.dispatch('categories/addCategory', data).then(resp => this.categories.push(resp))
      this.name = ''
    }
  },
};
